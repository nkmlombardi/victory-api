...
module.exports = (app) => {
    ...
    app.route('/v1/accounts')
        .get(services.authentication.isJwt,
            handlers.controller(controllers.account.getCollection, (request) => [request.user])
        )

    app.route('/v1/transactions')
        .get(services.authentication.isJwt,
            handlers.controller(controllers.transaction.getCollection, (request) => [request.user, request.query || []])
        )
