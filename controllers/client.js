...
module.exports = {
    ...
    find: async(request, response) => {
        try {
            ...
            return request.handlers.error(error, request, response)
        }

        if (response.query.length === 0) return request.handlers.error(4001, request, response)
        response.json({
            data: transformers.clients.collection(response.query)
        })
    },
    ...
    findAll: async(request, response) => {
        ...
        if (response.query.length === 0) return request.handlers.error(4001, request, response)
        response.json({
            data: transformers.clients.singleton(response.query[0])
        })
    },
    ...
    getOrigins: async(request, response) => {
        ...
        return response.json({
            data: transformers.origins.collection(response.query)
        })
    }
}
